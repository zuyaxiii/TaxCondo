...
const InputForm: React.FC<InputFormProps> = ({ values, setValues }) => {
  ...
  const handleChange = (
    ...
    if (name === "salePrice" || name === "assessedPrice" || name === "loanAmount") {
      ...
      const commasBeforeCursor = (value.substring(0, cursorPos).match(/,/g) || []).length;
      ...
      setTimeout(() => {
        ...
        const newCommasBeforeCursor = (formattedSubstr.match(/,/g) || []).length;
  ...
  return (
    <div className="mb-6">
      <h2 className="font-prompt font-light text-xl mb-4">ข้อมูลอสังหาริมทรัพย์</h2>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-4">
        <div>
          ...
          {isFieldInvalid("salePrice") && (
            <p className="text-red-500 text-sm mt-1">กรุณากรอกราคาซื้อขาย</p>
          )}
        </div>
        ...
        <div>
          ...
          {isFieldInvalid("assessedPrice") && (
            <p className="text-red-500 text-sm mt-1">กรุณากรอกราคาประเมิน</p>
          )}
        </div>
        ...
        <div>
          ...
          {isFieldInvalid("purchaseDate") && (
            <p className="text-red-500 text-sm mt-1">กรุณาเลือกวันที่เริ่มถือครอง</p>
          )}
        </div>
        ...
        <div>
          ...
          {isFieldInvalid("saleDate") && (
            <p className="text-red-500 text-sm mt-1">กรุณาเลือกวันที่โอนกรรมสิทธิ์</p>
          )}
        </div>
        ...
        <div>
          ...
          {isFieldInvalid("loanAmount") && (
            <p className="text-red-500 text-sm mt-1">กรุณากรอกยอดเงินกู้ (หากไม่มีให้ใส่ 0)</p>
          )}
        </div>
        ...
        <div className="flex flex-col justify-end col-span-2">
          ...
          <div className="border border-gray-300 bg-gray-100 p-3 rounded-b-lg col-span-2">
            <p>
              การคำนวณปีให้นับแบบวันชนวัน เช่น ผู้ขายมีชื่อบนโฉนด 1 มิ.ย. 2561
              (2018) ต้องโอนหลังจาก 1 มิ.ย. 2562 (2019) ถึงจะนับเป็น 1 ปี คือ
              ต้องโอนตั้งแต่วันที่ 2 มิ.ย. 2562 (2019) เป็นต้นไป
            </p>
          </div>
        </div>
      </div>
